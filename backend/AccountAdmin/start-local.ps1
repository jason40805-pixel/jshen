...
if (-not (Test-Path -LiteralPath (Join-Path $env:ADMIN_DATA_DIR 'accounts.json'))) {
    $bootstrapUser = Read-Host '首次建立的管理員帳號（預設 admin；3–64 位英數字）'
    $env:ADMIN_BOOTSTRAP_USER = if ([string]::IsNullOrWhiteSpace($bootstrapUser)) { 'admin' } else { $bootstrapUser.Trim() }
