// Opt-in live test: requires the local relay and dedicated AB credentials.
...
const health = () => fetch('http://127.0.0.1:5091/health/ab').then(r => r.json());
async function connect() {
  const start = Date.now();
  const response = await fetch('http://127.0.0.1:5091/api/ab/start', {
  ...
  return new Promise((resolve, reject) => {
    const ws = new WebSocket('ws://127.0.0.1:5091/ws/ab', { origin: 'http://localhost:3000' });
    ...
    ws.on('message', bytes => {
      ...
      if (message.type === 'tables') {
        clearTimeout(timer);
        resolve({ ws, count: message.tables.length, elapsed: Date.now() - start, sample: message.tables.slice(0,8).map(t=>({name:t.tableName,round:t.playId,results:t.results?.length,countdown:t.countDown})) });
      }
    });
  });
}
...
console.log(JSON.stringify({ sample:first.sample, firstTables: first.count, firstMs: first.elapsed, secondTables: second.count,
